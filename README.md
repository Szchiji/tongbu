...
tongbu 支持两种运行模式，请根据需求选择：

| 模式 | 适用场景 | 同步机器人消息 |
|------|---------|--------------|
| **Bot 账户模式**（默认） | 只需同步普通用户消息 | ❌ |
| **用户账户模式** | 需要同步其他机器人发送的内容 | ✅ |

- **Bot 账户模式**：设置 `BOT_TOKEN` 即可，配置简单，适合大多数场景。
- **用户账户模式**：不设置 `BOT_TOKEN`，改用个人账号的 Session 字符串，可接收群内所有消息（包括其他机器人）。

> 📖 详细的模式对比和配置说明请参见 [USER_ACCOUNT_MODE.md](USER_ACCOUNT_MODE.md)
...
> 💡 可参考仓库中的 [.env.example](.env.example) 文件，其中包含所有变量的详细注释。
...
> 📖 详细配置步骤请参见 [USER_ACCOUNT_MODE.md](USER_ACCOUNT_MODE.md)
